...
const contaierDiv = document.getElementById("contaierDiv");
const badge = document.getElementById("badge");
const cartItems = document.getElementById("cartItems");

const getlistProduct = () => JSON.parse(localStorage.getItem("productRec")) || [];

const updateBadge = () => {
  const listProduct = getlistProduct();
  if (badge) {
    badge.innerText = listProduct.reduce((sum, item) => sum + (item.qty || 1), 0);
  }
};
...
const selectProduct = (id) => {
  ...
  if (product) {
    const existingProduct = listProduct.find((prod) => prod.id === id);
    if (existingProduct) {
    ...
    localStorage.setItem("productRec", JSON.stringify(listProduct));
    updateBadge();
  }
};
...
const incQty = (id) => {
  let listProduct = getlistProduct();
  const product = listProduct.find((pro) => pro.id == id);
...
const deleteCart = (id) => {
  let listProduct = getlistProduct();
  listProduct = listProduct.filter((pro) => pro.id !== id);
  localStorage.setItem("productRec", JSON.stringify(listProduct));
  viewCart();
  updateBadge();
};
...
const clearCart = () => {
  localStorage.removeItem("productRec");
  const cartItems = document.getElementById("cartItems");
  if (cartItems) cartItems.innerHTML = "Your cart is empty.";
  updateBadge();
  updateTotal();
};
...
const updateTotal = () => {
  const listProduct = getlistProduct();
  const totalPrice = listProduct.reduce((sum, item) => sum + item.price * (item.qty || 1), 0);
  ...
  if (cartItems) {
    ...
    listProduct.forEach((item, index) => {
      ...
      if (item.images.length > 1) {
        // Use Bootstrap carousel for slider
        imageContent = `
        ...
        // Single image view
        imageContent = `<img src="${item.images[0]}" class="img-fluid rounded-start card-img-top" alt="${item.title}">`;
      }
            ...
            <div class="row g-0">
                ...
                <div class="col-md-8">
                    <div class="card-body">
                        ...
                        <p class="card-text">$${item.price.toFixed(2)} x ${item.qty}</p>
                        <p class="card-text">Rating: ${item.rating}</p>
                        <p class="card-text">Created At : ${item.meta.createdAt}</p>
                        <p class="card-text">Updated At :  ${item.meta.updatedAt}</p>
                        <button class="btn btn-warning" onclick="incQty(${item.id})">+</button>
                        <button class="btn btn-warning" onclick="decQty(${item.id})">-</button>
                        <button class="btn btn-danger" onclick="deleteCart(${item.id})">Remove</button>
                    </div>
                </div>
            </div>
        </div>`;
...
const viewSelection = () => {
  if (contaierDiv) { // Fixed typo
    ...
    products.forEach((item) => {
      ...
      const rating = stars(item.rating || 0);
        ...
        <div class="col-12 col-md-4">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <button class="btn btn-primary" onclick="selectProduct(${item.id})">Add to Bag</button>
                </div>
            </div>
        </div>`;
    });
    contaierDiv.innerHTML = htmlContent;
  }
};
...
const stars = (rating) => {
  ...
  for (let i = 0; i < filledStars; i++) {
    starShow += `<i class="fa-solid fa-star"></i>`;
  }
  if (hasHalfStar) {
    starShow += `<i class="fa-solid fa-star-half-stroke"></i>`;
  }
  for (let i = 0; i < emptyStars; i++) {
    starShow += `<i class="fa-regular fa-star"></i>`;
  }
  return starShow;
};
